...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 27 10:07:13 2020
...
#*****************************************************************************
file_path = r'/Users/jbs/Documents/Webscrape/'
...
bankruptcy_data.drop_duplicates(['gvkey', 'datadate'], inplace = True)
...
MDA_search_list = bankruptcy_data[['cik','datadate_year']].copy()
...
j_limit=MDA_search_list.shape[0]+1     #the max number of MDA sections to save
k=0             #counter for max number of MDA sections to process
k_limit = 3*MDA_search_list.shape[0]  #the max number of MDA sections to process
...
#Import some files that we need later when processing the MDA sections. We only need
#to import these files once, so I do not want this code inside the for loops below.
#The first file I am opening is a file that contains stop words (StopWords.txt). I use
#the with construct to open this file (when using with the file is automatically 
#closed after we have used it).

#Read each line from StopWords.txt into a list and remove new line characters (\n). 

with open(r'/Users/jbs/Documents/Webscrape/StopWords.txt') as f:
   stop_words = f.readlines()
stop_words = [words.rstrip() for words in stop_words]
...
for Y in range(MDA_search_list.datadate_year.min(), MDA_search_list.datadate_year.max()+1): 
    ...
    for Q in range(1,5):
        ...
        URLs = pd.read_fwf('https://www.sec.gov/Archives/edgar/full-index/' + str(Y) + '/QTR' + str(Q) + '/crawler.idx', colspecs=[(0,60), (60,72), (72,84), (84,96), (96,200)], skiprows=6)
        ...
        session = HTMLSession()
        CIK_list =[]
        for company in URLs[0:40].index:
            ...
            #I, however, decided to comment out the first approach as it appears that it occasionally 
            #returned the entire financial statement (I still like the code so I included it for your 
            #reference!) 
            ...
            for i, row in enumerate(tr_list): 
                if re.search('Management.{1,12}Discussion', str(row), re.IGNORECASE | re.DOTALL):
                    ...
                    if tr_list[MDARow].a and tr_list[NextSectionRow].a: 
                         start_anchor_name = tr_list[MDARow].a.get('href')
                         end_anchor_name = tr_list[NextSectionRow].a.get('href')
            ...
            if start_anchor_name and end_anchor_name:
                #Before createing the search pattern, I need to remove a character from the anchors.
                #The a href tag contains a character before the actual anchor name that is not 
                #part of the anchor name. So need to remove this character.
                start_anchor_name = start_anchor_name[1:]
                end_anchor_name = end_anchor_name[1:]
                
                MDA = re.findall("a name=.{1,5}" + re.escape(str(start_anchor_name)) + ".*" + "a name=.{1,5}" + re.escape(str(end_anchor_name)), str(financials_soup), re.I | re.DOTALL)
...
# *********** Approach 2 *************
            ...
            #  "(?!.{1,200}item)"        Followed by a negative lookahead that is saying that we do not want to see the word item within 1 to 200 characters of (item.{1,12}management.{0,6}s? discussion 
            ...
            #                            the right of ?.  This means that regex will return everything until the first occurrence of the next patter (rather than the last occurrence, which is what happens 
            ...
            #To only return full matches (and not all subgroups), each “or” group must 
            #be prefixed with ?:.
            #However, in this solution I instead use look ahead that just examines 
            #if one of these words exists.
            ...
            elif re.findall("(item.{0,2}[0-9]{1,2}\..{1,12}management.{0,6}s? discussion(?!.{1,200}item).*?(\..{1,10}item.{1,10}(quantitative|financial statement)))", financials_text, re.I | re.DOTALL):  
...
# *********** Approach 3 *************
            ...
            elif re.findall("(item.{0,2}[0-9]{1,2}\..{1,12}management.{0,6}s? discussion(?!.{1,200}item).*?(\..{1,10}item))", financials_text, re.I | re.DOTALL):
...
# start of MDA text and then bolded Item to indicate the end of the text. However, most code is processed
# using approach 1 and I think approach 2 is accurate enough for now...
...
#********************************************************************************
            if len(MDA_text)>500:
               #1) Normalize the MDA text using unicodedata.normalize. This step 
               #should resolves some of the items in the next step, but you will still 
               #replace a few items manually to make sure they are removed correctly. 
               MDA_text = unicodedata.normalize("NFKD", MDA_text)
               
               #2) 
               #Replace the following escape sequences with a single space (I cannot
               #use a character set, i.e., [], because I am trying to capture sequences 
               #of more two characters.
               
               #'\\' finds '\'               
               MDA_text = re.sub(r'\\xa0|\\n|\\s|\\r|\\t|\s', ' ', MDA_text) 
                  
               #Remove the following patters. #\\\'s finds \'s, \'s finds 's
               MDA_text = re.sub(r'\\xa|’s|\\\'s|\'s', '', MDA_text, re.I) 
  
               #Find all periods and then use look ahead to see if the next character 
               #is a letter, e.g., .On July 27..., then replace the period with a 
               #period and a space
               MDA_text = re.sub('\.(?=[a-zA-Z])', '. ', MDA_text) 

               #Find numbers and then use look ahead to see if the next charachter is a 
               #letter, e.g., 27We did..., then replace the number with a white space
               MDA_text = re.sub('[0-9]+(?=[a-zA-Z])', ' ', MDA_text) 

               #Remove the “a name=” tag 
               MDA_text = re.sub('.{2}a name=.{1,100}?>', '', MDA_text, re.I)

               #Remove all short text within parentheses. Parentheses in regex are 
               #used to indicate sub groups, so need to escape them.
               MDA_text = re.sub('\(.{0,20}?\)', ' ', MDA_text, re.I | re.DOTALL) 

               #Remove all slashes (\), less than signs (<), and more than signs (>)      
               #and all the characters immediately to the right until the next space 
               #the ? makes the .* lazy to make it stop at the first space after the \, 
               #<, or >, e.g., in the text '\We had' the expression matches \We
               MDA_text = re.sub(r'[\\|<|>].*? ', ' ', MDA_text, re.I | re.DOTALL)               
                        
               #All caps words followed by a space, period, or comma.
               #Find a space then 1-30 capital letters, and then a space, comma or 
               #period. Replace by nothing. The ?: within the or statement makes regex
               #not return multiple matches.
               MDA_text = re.sub(r' [A-Z]*[ ,\.]', '', MDA_text)
            
               #All words with the first letter being upper case and all other words 
               #being lower case, which indicates a name, 
               #header, or something similar (unless after a period, which would 
               #indicate the beginning of a sentence). Do negative lookbehind (?<!) and 
               #exclude all instances where the search pattern is preceeded by a 
               #period (\.). Then find all words (\b represents word boundaries, I 
               #cannot use space because then I will not find consecutive 
               #capitalized words) that starts with one capital letter and then an  
               #unlimited number of non-capital letters.
               MDA_text = re.sub(r'(?<!\.\s)\b[A-Z][a-z]*?\b', ' ', MDA_text)
                 
               #Remove the words million and per share
               MDA=re.sub('(?:million|per share)', ' ', MDA_text)

               #Make all words upper case 
               MDA_text=MDA_text.upper()
            
               #Keep only letters and spaces
               MDA_text=re.sub('[^A-Z ]+', ' ', MDA_text)

               #Remove single letter words
               MDA_text=re.sub(r'\b[A-Z]\b', '', MDA_text)

               #Replace multiple spaces with a single space
               MDA_text = re.sub('\s+', ' ', MDA_text)
               
               #Tokenize the MDA text and store the individual words in a list named 
               #MDA_Tokens.
               MDA_list = re.split(r'\s+', MDA_text)

               #****** Remove Stop Words from MDA Text ******
               #I earlier creates a list of stop words named stop_words. I will now use 
               #this list to remove works in the MDA_list that are part of the 
               #stop_words list
               

               #Pandas and numpy have isin functions that makes comparing to lists
               #quick and easy, but I first need to convert the MDA list to a numpy
               #array so that I can do boolean indexing (isin returns a boolean array).
               #I then convert the numpy array back to a list.
               MDA_list = np.array(MDA_list)
               MDA_list = MDA_list[np.isin(MDA_list, stop_words, invert=True)]
               MDA_text = ' '.join(MDA_list)
               MDA_list = MDA_list.tolist()

               #As an alternative to using isin, I could have used list comprehension, 
               #but vectorized operations are better when possible
               #MDA_Tokens = [word for word in MDA_Tokens if word not in stop_words]

            # *********** Keeping Track of Number of Loops and Saving MDA Sections *************
        
            #I am exiting the loop when I have found j_limit MDA sections that have at least 500 characters or if k_limit financials 
            #have been processed. The code only saves the MDA section if it has at least 500 characters. j_limit is currently the 
            #number of firm years in the MDA_search_list +1 and k_limit is 3 times j_limit.
            ...
            if len(MDA_text)>500:
                ...
                #Saving the data. This turned out to be more difficult then anticipated
                #I would probably have gone back and changed the loop to make this 
                #easier rather than creating the work around solution below. Also,
                #the drop duplicates code should be done earlier so that we do not need
                #to process the same company multiple times (these are addendums, we 
                #should just process the most recent in the period).

                bankruptcy_data.loc[(bankruptcy_data.cik == URLs.iloc[company].cik) & (bankruptcy_data.datadate_year == URLs.iloc[company].datadate_year), 'MDA_Text'] = MDA_text

                index_value = bankruptcy_data.loc[(bankruptcy_data.cik == URLs.iloc[company].cik) & (bankruptcy_data.datadate_year == URLs.iloc[company].datadate_year)].index[0]

                bankruptcy_data.at[index_value,'MDA_List']=MDA_list


                #If the saved MDA text is an addendum then the code needs to reduce j by one to make sure that we save the correct number of files that we want.  I create a list to keep track of 
        ...
        #Need these so that the outer loops do not run one extra time at the end after hitting the limits.
        if ((j>=j_limit) | (k>=k_limit)):
            break
    if ((j>=j_limit) | (k>=k_limit)):
        break
...
Sentiment_Dict =  pd.read_csv(r'/Users/jbs/Documents/Webscrape/FS_Sentiment_Dictionary.csv')

#This is our first attempt at the four loop.    

Sentiment_Dict =  pd.read_csv(r'/Users/jbs/Documents/Webscrape/FS_Sentiment_Dictionary.csv', low_memory=False)


bankruptcy_data['ScoreNegative'] = ''

bankruptcy_data['ScorePositive'] = ''

bankruptcy_data['ScoreUncertainty'] = ''

bankruptcy_data['ScoreLitigious'] = ''

bankruptcy_data['ScoreConstraining'] = ''

bankruptcy_data['ScoreSuperflouous'] = ''

bankruptcy_data['ScoreInteresting'] = ''

bankruptcy_data['ScoreModal'] = ''
...
for i in range(0,25):   
    ...
    words = bankruptcy_data.loc[i,"MDA_List"]
    for word in words:
        wordindex = Sentiment_Dict.loc[(word == Sentiment_Dict['Word'])].index
        if wordindex.size > 0:
            wordindex = wordindex[0]
            
            scorenegative += Dictionary.loc[wordindex, 'Negative']
           
            bankruptcy_data.at[i,'ScoreNegative'] = scorenegative
          
            scorepositive += Dictionary.loc[wordindex, 'Positive']
            
            bankruptcy_data.at[i,'ScorePositive'] = scorepositive
           
            scoreuncertainty += Dictionary.loc[wordindex, 'Uncertainty']
           
            bankruptcy_data.at[i,'ScoreUncertainty'] = scoreuncertainty
            
            scorelitigious += Dictionary.loc[wordindex, 'Litigious']
           
            bankruptcy_data.at[i,'ScoreLitigious'] = scorelitigious

            scoreconstraining += Dictionary.loc[wordindex, 'Constraining']
            
            bankruptcy_data.at[i,'ScoreConstraining'] = scoreconstraining

            scoresuperflouous += Dictionary.loc[wordindex, 'Superfluous']
           
            bankruptcy_data.at[i,'ScoreSuperflouous'] = scoresuperflouous
                
            scoreinteresting += Dictionary.loc[wordindex, 'Interesting']
           
            bankruptcy_data.at[i,'ScoreInteresting'] = scoreinteresting
            
            scoremodal += Dictionary.loc[wordindex, 'Modal']
           
            bankruptcy_data.at[i,'ScoreModal'] = scoremodal
            
        
        
#Standardization through null drops and the eq listed in the doc.

bankruptcy_data[['ScoreNegative','ScorePositive', 'ScoreUncertainty', 'ScoreLitigious', 'ScoreConstraining' , 'ScoreSuperflouous', 'ScoreInteresting', 'ScoreModal']] =  bankruptcy_data[['ScoreNegative','ScorePositive', 'ScoreUncertainty', 'ScoreLitigious', 'ScoreConstraining', 'ScoreSuperflouous', 'ScoreInteresting', 'ScoreModal']].replace("", np.nan)

bankruptcy_data['ScoreNegative'] = bankruptcy_data['ScoreNegative'] - bankruptcy_data(['ScoreNegative'].mean())/bankruptcy_data['ScoreNegative'].std

bankruptcy_data['ScorePositive'] = bankruptcy_data['ScorePositive'] - bankruptcy_data['ScorePositive'].mean()/bankruptcy_data['ScorePositive'].std

bankruptcy_data['ScoreUncertainty'] = bankruptcy_data['ScoreUncertainty'] - bankruptcy_data['ScoreUncertainty'].mean()/bankruptcy_data['ScoreUncertainty'].std
    
bankruptcy_data['ScoreLitigious'] = bankruptcy_data['ScoreLitigious'] - bankruptcy_data['ScoreLitigious'].mean()/bankruptcy_data['ScoreLitigious'].std
    
bankruptcy_data['ScoreConstraining'] = bankruptcy_data['ScoreConstraining'] - bankruptcy_data['ScoreConstraining'].mean()/bankruptcy_data['ScoreConstraining'].std
    
bankruptcy_data['ScoreSuperflouous'] = bankruptcy_data['ScoreSuperflouous'] - bankruptcy_data['ScoreSuperflouous'].mean()/bankruptcy_data['ScoreSuperflouous'].std
    
bankruptcy_data['ScoreInteresting'] = bankruptcy_data['ScoreInteresting'] - bankruptcy_data['ScoreInteresting'].mean()/bankruptcy_data['ScoreInteresting'].std
    
bankruptcy_data['ScoreModal'] = bankruptcy_data['ScoreModal'] - bankruptcy_data['ScoreModal'].mean()/bankruptcy_data['ScoreModal'].std


    
BData = bankruptcy_data.dropna(subset=['NegativeScore'])

lemmatize = WordNetLemmantizer()

BData['Len_MDA'] = ' '.join([lemmatize.lemmatize])
    
    
    
#lemmetize
for x in range(len(filledBD)):
    string = ' '.join([lemmatize.lemmatize(w) for w in BData.loc[x,'MDA_List']])
    BData.at[x,'Len_MDA' = string 

                

for i in range(len(words))):
    if words[i] in Sentiment_Dict["Words"]:
        print('Yes')
#stemmer 
...
for a in range(len(filledBD)):
    stemmer = ' '.join([stemming.stemmer(w) for w in BData.loc[a, 'MDA_List']])
    BData.at[a,'Stem_MDA'] = stem
...
for i in range(0,1):
    listnew = filledBD.loc[i,'MDA_Text'].split(" ")
   
    listlen = filledBD.loc[i,'Len_MDA'].split(" ")
   
    liststem = filledBD.loc[i,'Stem_MDA'].split(" ")
    
    X = vector.fit_transform(listnew)
    print(vectorizer.get_feature_names())
    
    Y = vector.fit_transform(listlen)
    print(vectorizer.get_feature_names())
    
    Z = vector.fit_transform(liststem)
    print(vectorizer.get_feature_names())
   
    
   svd = TruncatedSVD(2)
    tdidf_transformed = svd.fit_transform(ididf_data)
    tdidf_data[:10]
...
svd = TruncatedSVD()
tdidf_transformed = svd.fit_transfrom(X)
tdidf_transformed = tdidf_transformed[:10]
...
#This is where we were completley lost, around step 4 5 and 6, below are also other loop attempts for part one
...
X = vectorizer.fit_transform(newlist)
TokenDF = pd.DataFrame(X.toarray(), columns=vectorizer.get_feature_names())
svd = TruncatedSVD()
tdidf_transformed = svd.fit_transfrom(TokenDF)
tdidf_transformed = tdidf_transformed[:10]
...
#second
for obs in bankruptcy_data:
    words = pd.DataFrame(bankruptcy_data["MDA_List"][obs])
...
#third
for obs in range(len(bankruptcy_data)):
    words = pd.DataFrame(bankruptcy_data["MDA_List"] )
    for w in words:
        sentiments = []
        if w in Sentiment_Dict["Word"] & Sentiment_Dict["Word"][w] == 1:
            sentiments = Sentiment_Dict.columns[(Sentiment_Dict == 1).any(axis = 0)]


 sentiments = np.where(w == 1)
...
Sentiment_Dict.columns[(Sentiment_Dict[0:10] == 1)]



np.where(Sentiment_Dict == 1)
one = Sentiment_Dict[:1]   
